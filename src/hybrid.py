# -*- coding: utf-8 -*-
"""
Created on Wed Dec 16 15:49:32 2020
...
class datatest:
    ...
    def makehybridata(self):
        ...
        for i in range(0,len(self.counties)):
            ...
            #pdb.set_trace()
            hd=hybrid_data.reset_index()
            hd.to_csv('../output/Hybridata/US_hybridata_counties_std.csv', index=False)
    ...
    def comparison(self):
        ...
        dataset_health=pd.DataFrame(index=np.unique(self.nychealth_by_boro.index), columns=self.counties) 
        #pdb.set_trace()
        comparison=pd.DataFrame(index=np.unique(self.hybrid_data.index), columns=self.counties) 
        for i in range(0,5):
            dataset_health[self.counties[i]]= self.nychealth_by_boro[self.nychealth_by_boro.county==self.counties[i]].loc['2020-02-29':,'cases'].values
